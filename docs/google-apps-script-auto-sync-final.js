 ...
 * 
 * 変更点: 
 * - 元のスプレッドシートのURLは変更しない
 * - 新しいシート「コピー先URL」を作成し、そこに新しいURLを記録
 */
...
const FOLDER_URL_COLUMN = 2;  // C列（元のURL）
const TUTOR_NAME_COLUMN = 3;  // D列（講師名）

// 新しいシート名
const NEW_SHEET_NAME = 'コピー先URL';
...
function setupInitialSync() {
  ...
  // 新しいシート「コピー先URL」を作成または取得
  let destSheet = ss.getSheetByName(NEW_SHEET_NAME);
  if (!destSheet) {
    destSheet = ss.insertSheet(NEW_SHEET_NAME);
    Logger.log(`新しいシート「${NEW_SHEET_NAME}」を作成しました`);
    
    // ヘッダーを設定
    destSheet.getRange(1, 1, 1, 4).setValues([[
      '講師名',
      '元のフォルダURL',
      'コピー先フォルダURL',
      '最終更新日時'
    ]]);
    destSheet.getRange(1, 1, 1, 4).setFontWeight('bold');
  } else {
    Logger.log(`既存のシート「${NEW_SHEET_NAME}」を使用します`);
  }
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    try {
      ...
      if (!sourceFolderId) {
        Logger.log('  ❌ 無効なURL');
        results.push([tutorName, folderUrl, 'エラー: 無効なURL', new Date()]);
        continue;
      }
      ...
      // 結果を記録
      results.push([tutorName, folderUrl, newUrl, new Date()]);
      
      Logger.log(`  ✅ 完了`);
      
    } catch (error) {
      Logger.log(`  ❌ エラー: ${error.message}`);
      results.push([tutorName, folderUrl, `エラー: ${error.message}`, new Date()]);
    }
  }
  
  // 新しいシートにデータを書き込み
  if (results.length > 0) {
    destSheet.getRange(2, 1, results.length, 4).setValues(results);
    Logger.log(`\n「${NEW_SHEET_NAME}」シートに ${results.length} 件のデータを書き込みました`);
  }
  ...
  Logger.log(`\nスプレッドシートの「${NEW_SHEET_NAME}」シートを確認してください`);
}
...
function dailySync() {
  ...
  // 「コピー先URL」シートを取得
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const destSheet = ss.getSheetByName(NEW_SHEET_NAME);
  ...
  for (const sourceId in folderMapping) {
    ...
    try {
      ...
      if (stats.newFiles > 0) {
        ...
        // 「コピー先URL」シートの最終更新日時を更新
        if (destSheet) {
          const data = destSheet.getDataRange().getValues();
          for (let i = 1; i < data.length; i++) {
            if (data[i][0] === mapping.tutorName) {
              destSheet.getRange(i + 1, 4).setValue(new Date());
              break;
            }
          }
        }
